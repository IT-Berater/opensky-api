...
/**
 * Main class of the OpenSky Network API. Instances retrieve data from OpenSky
 * via HTTP
 *
...
public class OpenSkyApi {
	...
	public Response intercept(Chain chain) throws IOException {
	    Request req = chain.request().newBuilder().header("Authorization", credentials).build();
	...
	if (authenticated) {
	    okHttpClient = new OkHttpClient.Builder().addInterceptor(new BasicAuthInterceptor(username, password))
		    .build();
	} else {
    ...
    /**
     * Prevent client from sending too many requests. Checks are applied on
     * server-side, too.
     * 
     * @param type           identifies calling function (GET_STATES or
     *                       GET_MY_STATES)
     * @param timeDiffAuth   time im ms that must be in between two consecutive
     *                       calls if user is authenticated
     * @param timeDiffNoAuth time im ms that must be in between two consecutive
     *                       calls if user is not authenticated
     ...
     * @throws IOException if there was an HTTP error
     */
    private OpenSkyStates getOpenSkyStates(String baseUri, ArrayList<AbstractMap.Entry<String, String>> nvps)
	    throws IOException {
	try {
	    ...
	    throw new RuntimeException(
		    "Programming Error in OpenSky API. Could not parse JSON Data. Please report a bug");
	}
    }

    /**
     * Represents a bounding box of WGS84 coordinates (decimal degrees) that
     * encompasses a certain area. It is defined by a lower and upper bound for
	...
	/**
	 * Create a bounding box, given the lower and upper bounds for latitude and
	 * longitude.
	 */
    ...
    /**
     * Retrieve state vectors for a given time. If time == 0 the most recent ones
     * are taken. Optional filters might be applied for ICAO24 addresses.
     *
     * @param time   Unix time stamp (seconds since epoch).
     * @param icao24 retrieve only state vectors for the given ICAO24 addresses. If
     *               {@code null}, no filter will be applied on the ICAO24 address.
     * @return {@link OpenSkyStates} if request was successful, {@code null}
     *         otherwise or if there's no new data/rate limit reached
     * @throws IOException if there was an HTTP error
     */
     ...
     * Retrieve state vectors for a given time. If time == 0 the most recent ones
     * are taken. Optional filters might be applied for ICAO24 addresses.
     * Furthermore, data can be retrieved for a certain area by using a bounding
     * box.
     *
     * @param time   Unix time stamp (seconds since epoch).
     * @param icao24 retrieve only state vectors for the given ICAO24 addresses. If
     *               {@code null}, no filter will be applied on the ICAO24 address.
     * @param bbox   bounding box to retrieve data for a certain area. If
     *               {@code null}, no filter will be applied on the position.
     * @return {@link OpenSkyStates} if request was successful, {@code null}
     *         otherwise or if there's no new data/rate limit reached
     * @throws IOException if there was an HTTP error
     */
    ...
    /**
     * Retrieve state vectors for your own sensors. Authentication is required for
     * this operation. If time = 0 the most recent ones are taken. Optional filters
     * may be applied for ICAO24 addresses and sensor serial numbers.
     *
     * @param time    Unix time stamp (seconds since epoch).
     * @param icao24  retrieve only state vectors for the given ICAO24 addresses. If
     *                {@code null}, no filter will be applied on the ICAO24 address.
     * @param serials retrieve only states of vehicles as seen by the given sensors.
     *                It expects an array of sensor serial numbers which belong to
     *                the given account. If {@code null}, no filter will be applied
     *                on the sensor.
     * @return {@link OpenSkyStates} if request was successful, {@code null}
     *         otherwise or if there's no new data/rate limit reached
     * @throws IOException if there was an HTTP error
